...
export const usePasswordUpdate = (navigation: NativeStackNavigationProp<RootStackParamList, 'PasswordUpdate'>) => {
  ...
  const handleUpdateProfile = async (): Promise<void> => {
    ...
    try {
      await dispatch(updatePassword({ oldPassword: state.oldPassword, newPassword: state.newPassword })).unwrap();
      Alert.alert('Success', 'Password updated successfully', [{
        text: 'OK',
        onPress: () => {
          setState({ oldPassword: '', newPassword: '', confirmNewPassword: '', isLoading: false });
          navigation.goBack();
        },
      }]);
    } catch (error: any) {
